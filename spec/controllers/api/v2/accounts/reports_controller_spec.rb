...
RSpec.describe Api::V2::Accounts::ReportsController, type: :request do
  ...
  describe 'GET /api/v2/accounts/{account.id}/reports/performance' do
    let(:second_agent) { create(:user, account: account, role: :agent) }
    let(:range_params) do
      {
        since: 7.days.ago.beginning_of_day.to_i,
    ...
    before do
      ...
      create(:conversation, account: account, inbox: inbox, assignee: second_agent)
    end

    it 'returns only the authenticated agent performance by default' do
      get "/api/v2/accounts/#{account.id}/reports/performance",
          params: range_params,
          headers: agent.create_new_auth_token,
          as: :json

      expect(response).to have_http_status(:success)
      expect(response.parsed_body['scope']).to eq('agent')
      expect(response.parsed_body.dig('subject', 'id')).to eq(agent.id)
      expect(response.parsed_body.dig('queue', 'open')).to eq(1)
    end

    it 'does not allow an agent to select another agent' do
      get "/api/v2/accounts/#{account.id}/reports/performance",
          params: range_params.merge(agent_id: second_agent.id),
          headers: agent.create_new_auth_token,
          as: :json
    ...
    it 'does not allow an agent to request the account overview' do
      get "/api/v2/accounts/#{account.id}/reports/performance",
          params: range_params.merge(scope: 'account'),
          headers: agent.create_new_auth_token,
          as: :json
    ...
    it 'allows an administrator to request the account overview' do
      get "/api/v2/accounts/#{account.id}/reports/performance",
          params: range_params.merge(scope: 'account'),
      ...
      expect(response).to have_http_status(:success)
      expect(response.parsed_body['scope']).to eq('account')
      expect(response.parsed_body.dig('queue', 'open')).to eq(2)
    end
  end
end
