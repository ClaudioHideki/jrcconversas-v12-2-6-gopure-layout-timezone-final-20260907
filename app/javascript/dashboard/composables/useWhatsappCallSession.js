...
export function useWhatsappCallSession() {
  ...
  // target: { conversationId }, { contactId, inboxId }, or { phoneNumber, inboxId }
  const initiateOutboundCall = async target => {
    ...
    try {
      ...
      const functionalStatuses = Object.values(VOICE_CALL_OUTBOUND_INIT_STATUS);
      if (functionalStatuses.includes(data?.status)) {
        return data;
      }
      throw e;
    } finally {
