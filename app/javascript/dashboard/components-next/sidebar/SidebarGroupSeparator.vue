...
<template>
  <div class="relative min-w-0" :class="{ 'ms-5': collapsible }">
    <component
      ...
      :title="label"
      class="relative flex h-8 w-full min-w-0 items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-white select-none"
      :class="[
    ...
    >
      ...
      <button
        v-if="collapsible"
        type="button"
        class="flex size-6 flex-shrink-0 items-center justify-center rounded-md text-white hover:bg-white/10 hover:text-white focus-visible:bg-white/10 focus-visible:outline-none"
