...
const contactPhoneNumber = computed(
  () =>
    contactData.value?.phoneNumber || props.selectedContact?.phoneNumber || ''
);

const callContactByRamal = () => {
  const phone = String(contactPhoneNumber.value || '').replace(/[^\d+]/g, '');
  if (!phone) return;
  router.push({
    path: frontendURL(`accounts/${route.params.accountId}/ramal`),
    query: { contact_id: props.selectedContact.id, phone },
  });
};
