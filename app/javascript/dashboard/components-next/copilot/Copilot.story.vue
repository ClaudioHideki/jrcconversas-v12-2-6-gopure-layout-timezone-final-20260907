...
const isAssistente JRCTyping = ref(false);
...
const sendMessage = message => {
  ...
  isAssistente JRCTyping.value = true;
  setTimeout(() => {
    isAssistente JRCTyping.value = false;
    messages.value.push({
      id: messages.value.length + 1,
      role: 'assistant',
...
<template>
  <Story
    title="Assistente JRC/Copiloto JRC"
    :layout="{ type: 'grid', width: '400px', height: '800px' }"
  >
    <Copilot
      :support-agent="supportAgent"
      :messages="messages"
      :is-captain-typing="isAssistente JRCTyping"
