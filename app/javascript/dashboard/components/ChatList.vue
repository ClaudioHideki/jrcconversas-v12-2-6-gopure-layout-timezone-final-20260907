...
const assigneeTabItems = computed(() => {
  ...
  return filterItemsByPermission(
    ...
    .map(({ key, count: countKey }) => ({
      key,
      name: tabLabels[key] || t(`CHAT_LIST.ASSIGNEE_TYPE_TABS.${key}`),
...
function setFiltersFromUISettings() {
  ...
  const routeSortBy = route.query.sort_by;
  const routeAssigneeType = route.query.assignee_type;
  const savedSortBy = Object.values(wootConstants.SORT_BY_TYPE).includes(
    orderBy
  )
    ? orderBy
    : wootConstants.SORT_BY_TYPE.LAST_ACTIVITY_AT_DESC;

  activeStatus.value = Object.values(wootConstants.STATUS_TYPE).includes(
    routeStatus
  )
    ? routeStatus
  ...
  activeSortBy.value = Object.values(wootConstants.SORT_BY_TYPE).includes(
    ...
    ? routeSortBy
    : savedSortBy;
  activeAssigneeTab.value = Object.values(wootConstants.ASSIGNEE_TYPE).includes(
    routeAssigneeType
  )
    ? routeAssigneeType
    : activeAssigneeTab.value;
...
function syncFiltersFromRouteQuery() {
  const nextStatus = route.query.status;
  const nextSortBy = route.query.sort_by;
  const nextAssigneeType = route.query.assignee_type;

  if (Object.values(wootConstants.STATUS_TYPE).includes(nextStatus)) {
    activeStatus.value = nextStatus;
  }
  if (Object.values(wootConstants.SORT_BY_TYPE).includes(nextSortBy)) {
    activeSortBy.value = nextSortBy;
  }
  if (Object.values(wootConstants.ASSIGNEE_TYPE).includes(nextAssigneeType)) {
    activeAssigneeTab.value = nextAssigneeType;
  }
}
