...
import SalesOpportunityForm from 'dashboard/routes/dashboard/sales/SalesOpportunityForm.vue';
import WhatsappCallingPanel from './components/WhatsappCallingPanel.vue';
...
const isSalesEnabled = computed(() =>
  isCloudFeatureEnabled(FEATURE_FLAGS.SALES)
);
...
<template>
  <div class="w-full">
    ...
    />
    <WhatsappCallingPanel
      :contact="contact"
      :conversation-id="conversationId"
    />
    <ContactInfo :contact="contact" :channel-type="channelType" />
    <div v-if="isSalesEnabled" class="px-3 pb-3">
      <Button
        class="w-full"
        icon="i-lucide-kanban"
        :label="$t('SALES.CREATE_FROM_CONVERSATION')"
        @click="showOpportunityForm = true"
      />
    </div>
    <SalesOpportunityForm
      v-if="showOpportunityForm && contactId"
      :contact-id="contactId"
      :conversation-id="conversationId"
      @close="showOpportunityForm = false"
      @saved="showOpportunityForm = false"
    />
