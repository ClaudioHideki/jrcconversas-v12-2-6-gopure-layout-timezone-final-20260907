...
<template>
  ...
  >
    ...
    <template #body>
      ...
      <div
        v-else
        class="mx-auto w-full max-w-5xl divide-y divide-n-weak border-t border-n-weak"
      >
        <div
          v-for="(agent, index) in filteredAgentList"
          :key="agent.email"
          class="grid min-h-[72px] grid-cols-1 items-center gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_130px_130px_auto] sm:gap-6"
        >
          <div class="flex min-w-0 items-center gap-4">
            ...
            <div class="flex min-w-0 flex-col gap-1">
              <span
                class="block truncate text-sm font-semibold text-n-slate-12 capitalize"
              >
                {{ agent.name }}
              </span>
              <span class="truncate text-sm text-n-slate-10">
                {{ agent.email }}
              </span>
            </div>
          </div>
          <span
            class="relative block w-fit text-sm text-n-slate-11"
            :class="{
          ...
          >
            ...
            <div
              class="absolute ltr:left-0 rtl:right-0 z-10 hidden w-[300px] bg-n-alpha-3 backdrop-blur-[100px] rounded-xl outline outline-1 outline-n-container shadow-lg top-10"
          ...
          <span v-if="agent.confirmed" class="text-sm text-n-slate-11">
    ...
    <woot-modal v-model:show="showEditPopup" :on-close="hideEditPopup">
      <EditAgent
        ...
        :crm-enabled="currentAgent.crm_enabled"
        @close="hideEditPopup"
      />
    </woot-modal>
