class Api::V2::Accounts::ReportsController < Api::V1::Accounts::BaseController
  ...
  before_action :check_authorization, except: [:performance]
  ...
  def performance
    ...
    render json: V2::Reports::PerformanceBuilder.new(
      account: Current.account,
  ...
  def performance_user
    ...
    requested_user_id = params[:agent_id].presence || Current.user.id
    authorize :report, :view? if requested_user_id.to_i != Current.user.id
    Current.account.users.find(requested_user_id)
  end

  def performance_params
    params.permit(:since, :until, :group_by, :timezone_offset, :business_hours).to_h.symbolize_keys
  end
