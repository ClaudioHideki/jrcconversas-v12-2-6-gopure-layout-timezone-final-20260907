class SuperAdmin::AccountsController < SuperAdmin::ApplicationController
  ...
  def toggle_feature
    ...
    feature_name = params[:feature].to_s
    valid_features = SuperAdmin::AccountFeaturesHelper.account_features.pluck('name')
    raise ActionController::BadRequest, 'Invalid feature' unless valid_features.include?(feature_name)

    if account.feature_enabled?(feature_name)
    ...
    redirect_back(
      ...
      notice: "Funcionalidade #{feature_name} #{state}."
    )
  end
