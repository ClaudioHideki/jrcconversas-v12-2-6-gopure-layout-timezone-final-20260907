...
class Channel::Whatsapp < ApplicationRecord
  ...
  def voice_enabled?
    configuration = account.whatsapp_calling_configuration
    voice_calling_supported? &&
      provider_config['calling_enabled'].present? &&
      account.feature_enabled?('channel_voice') &&
      configuration&.ready? &&
      configuration.phone_number_id.to_s == provider_config['phone_number_id'].to_s
  end
  ...
  def enable_voice_calling!
    raise 'Somente caixas do provedor WhatsApp Cloud podem habilitar chamadas.' unless voice_calling_supported?
    raise 'WhatsApp calling requires the channel_voice feature' unless account.feature_enabled?('channel_voice')

    configuration = account.whatsapp_calling_configuration
    unless configuration&.ready? && configuration.phone_number_id.to_s == provider_config['phone_number_id'].to_s
      raise 'Esta caixa WhatsApp Cloud não corresponde ao Phone Number ID configurado no WhatsApp Calling.'
    end
  ...
  def disable_voice_calling!
    raise 'Somente caixas do provedor WhatsApp Cloud podem desabilitar chamadas.' unless voice_calling_supported?
