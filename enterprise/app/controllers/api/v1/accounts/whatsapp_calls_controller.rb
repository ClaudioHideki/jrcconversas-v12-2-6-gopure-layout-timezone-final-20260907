...
class Api::V1::Accounts::WhatsappCallsController < Api::V1::Accounts::BaseController
  ...
  before_action :set_call_context, only: %i[initiate permission request_permission]
  before_action :ensure_calling_enabled, only: %i[initiate permission request_permission]
  before_action :ensure_sdp_offer, only: :initiate
  before_action :ensure_contact_phone, only: %i[initiate permission request_permission]
  before_action :ensure_call_permission, only: :initiate
  ...
  rescue_from Voice::CallErrors::CallFailed, with: :render_provider_call_failed
  rescue_from Voice::CallErrors::CallAlreadyEnded, with: :render_call_ended
  rescue_from Voice::CallErrors::NoCallPermission, with: :render_permission_required
  rescue_from Voice::CallErrors::InvalidCallingCredential, with: :render_invalid_credential
  rescue_from Voice::CallErrors::PermissionCheckFailed, with: :render_permission_check_failed
  ...
  def permission
    render json: permission_payload
  end

  def request_permission
    permission = permission_payload
    return render json: permission if permission[:can_start_call]
    return render json: permission, status: :unprocessable_entity unless permission[:can_request_permission]
    ...
    render json: { status: status, can_start_call: false, can_request_permission: false,
  ...
  def set_call_context
    if params[:conversation_id].present?
      set_context_from_conversation
    elsif params[:contact_id].present?
      set_context_from_contact
    else
      set_context_from_phone_number
    end
  end
  ...
  def set_context_from_phone_number
    digits = params[:phone_number].to_s.gsub(/\D/, '')
    return render_could_not_create_error(I18n.t('errors.contacts.phone_number.invalid')) unless digits.match?(/\A[1-9]\d{7,14}\z/)

    @inbox = Current.account.inboxes.find(params[:inbox_id])
    authorize @inbox, :show?
    phone_number = "+#{digits}"
    @contact = Current.account.contacts.find_or_create_by!(phone_number: phone_number) { |contact| contact.name = phone_number }
    @conversation = conversation_builder.existing_conversation
    authorize(@conversation || conversation_builder.new_conversation, :show?)
  end
  ...
  def ensure_call_permission
    permission = permission_payload
    return if permission[:can_start_call]

    render json: permission, status: :unprocessable_entity
  end
  ...
  def permission_payload
    payload = provider_service.call_permission(@contact.phone_number.delete('+')).symbolize_keys
    if payload[:status] == 'permission_required' && permission_recently_requested?
      payload[:status] = 'permission_pending'
      payload[:can_request_permission] = false
    end
    payload
  end

  def permission_recently_requested?
    requested_at = @conversation&.additional_attributes&.dig('call_permission_requested_at')
    requested_at.present? && Time.zone.parse(requested_at) > 24.hours.ago
  rescue ArgumentError
    false
  end

  def render_permission_required
    render json: { status: 'permission_required', can_start_call: false, can_request_permission: false }, status: :unprocessable_entity
  end

  def render_invalid_credential(error)
    render json: { status: 'credential_invalid', error: error.message }, status: :unprocessable_entity
  end

  def render_permission_check_failed(error)
    render json: { status: 'permission_unavailable', error: error.message }, status: :service_unavailable
  end
  ...
  def render_provider_call_failed
    render json: { status: 'failed', error: I18n.t('errors.whatsapp.calls.call_failed') }, status: :unprocessable_entity
  end
